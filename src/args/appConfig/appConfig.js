...
function printAppConfigDetails() {
    ...
    clog.log(" - config object:", config);
    clog.log(" - Update 'userConfig' manually in config.js to match your environment (e.g. repo paths), then use this command to verify the details.");
    // print details of localAppConfigShadow
    clog.log("localAppconfigShadow file details:");
    clog.log(" - userHomePath:", userHomePath);
    clog.log(" - localAppConfigShadow path:", path.join(userHomePath, '__cyk', ...pkg.name.split('/'), 'config.js'));
}
